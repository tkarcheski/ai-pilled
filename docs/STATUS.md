...
| P1 | Finish installer parent-directory confinement | A disposable Codex installation probe reproduced outside-repository publication after a parent symlink swap. Codex manifest/config publication now uses rooted atomic writes; install/uninstall regression fixtures preserve outside files and the original directory. Both installer lock files now open relative to a verified state-directory descriptor; a swapped parent fails before lock creation or mutation entry. Codex configuration/manifest reads now use the shared bounded rooted reader, rejecting observed replacement, deletion, mode changes and symlink parents. Codex uninstall and failed-install manifest cleanup now hold the rooted parent across ownership checking and unlink; reproduced swaps preserve outside files. Git uninstall now repeats rooted manifest/hook validation after restoring Git configuration and before deleting files; edits observed during that operation preserve all hooks and recovery metadata, though Git configuration may already be restored. Remaining Git installer writes/removals and filename replacement between a check and mutation still need review; these bounded fixes do not certify an atomic transaction. |
