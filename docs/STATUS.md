...
| P1 | Finish installer parent-directory confinement | A disposable Codex installation probe reproduced outside-repository publication after a parent symlink swap. Codex manifest/config publication now uses rooted atomic writes; install/uninstall regression fixtures preserve outside files and the original directory. Raw reads, locks, rollback and Git installer paths still need a complete transaction review; this bounded fix does not certify them. |
