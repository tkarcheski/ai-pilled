...
| Validate every push | Destination protection, outgoing-history scans, required tests, snapshot verification | Active Git pre-push hook in development; tests cover protected refs, historical credentials, changed HEAD/files, missing evidence. Local hooks can be bypassed by users; server protections remain separate. |
