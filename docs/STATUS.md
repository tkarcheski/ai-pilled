...
| Validate every push | Destination protection, outgoing subject conventions and history scans, required tests, snapshot verification | Active Git pre-push hook in development; disposable local remotes verify successful updates and unchanged refs after protected-branch or failing-test blocks. Other fixtures cover historical credentials, changed snapshots, and missing evidence. Local hooks can be bypassed by users; server protections remain separate. |
| Dependency audit | npm vulnerability/health/license checks; Python pinned-requirements vulnerability auditing; opt-in npm post-tool audits | Real npm fixtures and malformed/offline tests. Python: pip-audit2.10.1 checked all9 development pins (no findings) and blocked a non-installed requests2.19.1 fixture with10 advisories. Only listed exact pins are audited; ranges, extras, markers, and included files require export first. Python health and license commands are implemented; automatic lifecycle integration is in progress. Real dev environment health checked10 installed packages and confirmed10 PyPI version queries with no conflicts or updates. License metadata matching is not legal analysis. |
...
Python package support, stronger security/code review, and GitHub end-to-end CI are active work. GitLab CI is planned for later at the user’s request. Remote GitHub workflow publication remains blocked by the current token’s missing workflow scope.
