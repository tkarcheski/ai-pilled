...
| P1 | Finish installer parent-directory confinement | A disposable Codex installation probe reproduced outside-repository publication after a parent symlink swap. Codex manifest/config publication now uses rooted atomic writes; install/uninstall regression fixtures preserve outside files and the original directory. Both installer lock files now open relative to a verified state-directory descriptor; a swapped parent fails before lock creation or mutation entry. Codex configuration/manifest reads now use the shared bounded rooted reader, rejecting observed replacement, deletion, mode changes and symlink parents. Rollback and other Git installer paths still need a complete transaction review; these bounded fixes do not certify them. |
